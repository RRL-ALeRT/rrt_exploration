...
int main(int argc, char **argv) {
  ...
  // ros::param::param<std::string>(ns+"/robot_frame", base_frame_topic,
  // "/robot_1/base_link");
  ...
  nh->declare_parameter<std::string>(ns + "/map_topic", "/map"); // /robot_1/map
  nh->get_parameter<std::string>(ns + "/map_topic", map_topic);

  nh->declare_parameter<std::string>(ns + "/robot_frame",
                                     "/base_link"); // /robot_1/base_link
  nh->get_parameter<std::string>(ns + "/robot_frame", base_frame_topic);
  //---------------------------------------------------------------
  ...
  tf2_ros::Buffer tf_buffer(nh->get_clock());
  tf2_ros::TransformListener listener(tf_buffer, nh);
  ...
  while (rclcpp::ok()) {
    ...
    RCLCPP_INFO_STREAM(nh->get_logger(),
                       "Nearest" << x_nearest[0] << ", " << x_nearest[1]);
      ...
      while (temp == 0) {
        try {
          ...
          tf_buffer.lookupTransform(
              map_topic.substr(1), base_frame_topic.substr(1), rclcpp::Time(0));
          // listener.lookupTransform(map_topic, base_frame_topic ,
          // rclcpp::Time(0), transform);
