...
- ```clicked_point``` ([geometry_msgs/PointStamped Message](http://docs.ros.org/api/geometry_msgs/html/msg/PointStamped.html)): The ```lobal_rrt_frontier_detector``` also create_subscriptions to this topic similar to the global_rrt_frontier_detector. 
...
The filter node create_subscriptions for all the costmap topics of all the robots, the costmap is required therefore. Normally, costmaps should be published by the navigation stack (after bringing up the navigation stack on the robots, each robot will have a costmap).
For example, if  ```n_robots=2```, the node will create_subscription to:
